...
Button::Button(char* imgPath, std::string text, std::string font, int ptsize, const SDL_Color &font_color, const SDL_Rect &target_rect, const SDL_Rect &rect, const SDL_Color &color, bool is_active, int id)
    :  b_txt { text, font, ptsize, font_color, target_rect }, b_rect { rect.x, rect.y, rect.w, rect.h }, b_color { color.r, color.g, color.b, color.a }, b_state { is_active }, b_id { id }
{
    if (!imgPath)
    {
        std::cout << "Call to generateBttTexture with argument: " << imgPath << '\n'; 
        int control = generateBttTexture(imgPath);
        std::cout << "Returned from generateBttTexture with flag " << control << '\n';
    }
}


void Button::assign(char* imgPath, std::string text, std::string font, int ptsize, const SDL_Color &font_color, const SDL_Rect &target_rect, const SDL_Rect &rect, const SDL_Color &color, bool is_active, int id)
{
    std::cout << "Call to button.assign().\n";

    b_txt.assign(text, font, ptsize, font_color, target_rect);
    b_rect = rect;
    b_color = color;
...
int Button::generateBttTexture(char *imgPath)
{
    if(b_texture)
    {
